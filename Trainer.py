...
from Methods.FasterGSVDA.Loss import FasterGSVDALoss
from Methods.FasterGSVDA.utils import enable_expandable_segments, carve
...
@Framework.Configurable.configure(
    ...
    OPTIMIZER=Framework.ConfigParameterList(
        ...
        APPEARANCE_COSINE_DECAY=Framework.ConfigParameterList(
            USE=False,  # cosine-decays the base color + residual param lrs (SV instead: sites + temperatures, all colors constant like its reference)
...
class FasterGSVDATrainer(GuiTrainer):
    """Defines the trainer for the FasterGSVDA variant."""
    ...
    def setup_gaussians(self, _, dataset: 'BaseDataset') -> None:
        ...
        if self.OPTIMIZER.AUTO_OPACITY_LEARNING_RATE:
            # scenes with training views further apart get a larger opacity learning rate
            ...
            distance_ratio = 0.3536 / mean_knn_distance
            self.OPTIMIZER.LEARNING_RATE_OPACITIES = min(max(0.025 * distance_ratio ** -0.6, 0.025), 0.05)
            Logger.log_info(f'auto opacity lr from mean camera distance {mean_knn_distance:.4f}: {self.OPTIMIZER.LEARNING_RATE_OPACITIES:.3e}')
        self.model.gaussians.appearance.auto_scale_learning_rates(self.OPTIMIZER, camera_centers)  # no-op for anything but NASG/NASGabor
        ...
        self.loss = FasterGSVDALoss(loss_config=self.LOSS, model=self.model)

    @training_callback(priority=110, start_iteration=1000, iteration_stride=1000)
    @torch.no_grad()
    def increase_appearance_degree(self, *_) -> None:
        """Increase the used appearance degree up to the maximum."""
        self.model.gaussians.increase_appearance_degree()
