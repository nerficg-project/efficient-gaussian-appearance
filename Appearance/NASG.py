"""FasterGSVDA/Appearance/NASG.py"""
...
class NASG(AppearanceModel):
    """View-dependent appearance via normalized anisotropic spherical Gaussian lobes."""

    def __init__(self, appearance_config: Framework.ConfigParameterList) -> None:
        super().__init__(appearance_config)
        self.max_degree = self.appearance_config.NASG.N_LOBES

    def _initialize_residual_parameters(self, n_primitives: int) -> None:
        """Creates and registers the initial NASG lobe parameters."""
        residual_params = torch.empty((n_primitives, self.appearance_config.NASG.N_LOBES, 8), dtype=torch.float32, device='cuda')
        ...
        residual_params[..., 5:8] = 0.5            # rgb lobe weights: amplitude tanh(0.5) = 0.46 with the reference tanh residual activation
    ...
    def residual_colors(self, directions: torch.Tensor) -> torch.Tensor:
        """Evaluates the active NASG lobes for all Gaussians (N, 3)."""
        ...
        pdf = torch.where(v_z >= 0.99999988, 1.0, torch.where(v_z > -0.99999988, p * E * N, 0.0))
        lobe_colors = self.residual_activation(self._residual_params[:, :self.active_degree, 5:8])
