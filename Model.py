...
from Methods.FasterGSVDA.Appearance import create_appearance_model
from Methods.FasterGSVDA.FasterGSVDACudaBackend import FusedAdam, update_3d_filter, relocation_adjustment, add_noise
...
class Gaussians(torch.nn.Module):
    ...
    def __init__(self, appearance_config: Framework.ConfigParameterList) -> None:
        super().__init__()
        self.appearance = create_appearance_model(appearance_config)
    ...
    @property
    def raw_appearance_parameters(self) -> dict[str, torch.Tensor]:
    ...
    def increase_appearance_degree(self) -> None:
        """Increases the used appearance degree."""
        self.appearance.increase_degree()
    ...
    def initialize_from_point_cloud(self, point_cloud: BasicPointCloud, use_mcmc: bool) -> None:
        ...
        self.appearance.initialize_parameters(n_initial_gaussians, point_cloud.colors)
        # setup parameters
    ...
    def training_setup(self, training_wrapper, training_cameras_extent: float) -> None:
        ...
        self.appearance.setup_optimizer(training_wrapper.OPTIMIZER)

    def update_learning_rate(self, iteration: int) -> None:
        """Computes the current learning rates for the given iteration."""
        self.lr_means = self.lr_means_scheduler(iteration)
        for param_group in self.optimizer.param_groups:
            if param_group['name'] == 'means':
                param_group['lr'] = self.lr_means
        self.appearance.update_learning_rate(iteration)

    def optimizer_step(self) -> None:
        """Performs an optimizer step on all Gaussian parameters."""
        self.optimizer.step()
        self.optimizer.zero_grad()
        self.appearance.optimizer_step()
    ...
    def _assign_parameters(self, param_groups: dict[str, torch.nn.Parameter]) -> None:
        """Assign the given Gaussian parameters."""
    ...
    def prune(self, prune_mask: torch.Tensor) -> None:
        ...
        self._assign_parameters(prune_param_groups(self.optimizer, valid_mask))
        self.appearance.prune_parameters(valid_mask)
    ...
    def sort(self, ordering: torch.Tensor) -> None:
        ...
        self._assign_parameters(sort_param_groups(self.optimizer, ordering))
        self.appearance.sort_parameters(ordering)
    ...
    def adaptive_density_control(self, grad_threshold: float, min_opacity: float, prune_large_gaussians: bool) -> None:
        ...
        duplicated_appearance = self.appearance.select_parameters(duplicate_mask)
        ...
        split_appearance = self.appearance.select_parameters(split_mask, n_copies=2)

        # incorporate
        n_new_gaussians = n_new_gaussians_duplicate + n_new_gaussians_split
        self._assign_parameters(extend_param_groups(self.optimizer, {
        ...
        self.appearance.extend_parameters(duplicated_appearance, split_appearance)
    ...
    def mcmc_densification(self, min_opacity: float, cap_max: int) -> None:
        ...
        if n_dead_gaussians > 0:
            ...
            self.appearance.copy_parameters(dead_indices, sampled_indices)

            # reset optimizer state for the sampled Gaussians
            reset_state(self.optimizer, indices=sampled_indices)
            self.appearance.reset_optimizer_state(sampled_indices)
        ...
        if n_added_gaussians > 0:
            ...
            sampled_appearance = self.appearance.select_parameters(sampled_indices)
            self._assign_parameters(extend_param_groups(self.optimizer, {
            ...
            self.appearance.extend_parameters(sampled_appearance)
    ...
    def training_cleanup(self, min_opacity: float) -> int:
        ...
        # clear any leftover gradients and delete the optimizers
        self.optimizer.zero_grad()
        self.optimizer = None
        self.appearance.teardown_optimizer()
    ...
    def as_ply_dict(self) -> dict[str, np.ndarray]:
        ...
        appearance_attributes, appearance_attribute_names = self.appearance.ply_attributes()
        ...
        attributes = np.concatenate((means, appearance_attributes, opacities, scales, rotations), axis=1)

        # construct structured array
        attribute_names = (
              ['x', 'y', 'z']                         # 3d mean
            + appearance_attribute_names              # appearance model parameters
...
@Framework.Configurable.configure(
    APPEARANCE=Framework.ConfigParameterList(
        TYPE='SH',  # class name of an appearance model from Methods/FasterGSVDA/Appearance ('SH', 'SV', 'NASG', 'NASGabor', 'Neural')
        BASE_ACTIVATION='none',  # 'none' or 'exp' (exp(3x))
        RESIDUAL_ACTIVATION='none',  # 'none', 'tanh', or 'softplus' (beta=10)
        ...
        SH=Framework.ConfigParameterList(
            SH_DEGREE=3,
        ),
        SV=Framework.ConfigParameterList(
            N_SITES=7,  # reference default with Gaussian kernels (the beta-kernel variant uses 8)
        ),
        NASG=Framework.ConfigParameterList(
            N_LOBES=1,
        ),
        NASGABOR=Framework.ConfigParameterList(
            N_LOBES=1,
        ),
        NEURAL=Framework.ConfigParameterList(
            FEATURE_DIM=8,
...
class FasterGSVDAModel(BaseModel):
    """Defines the FasterGSVDA model."""
    ...
    def build(self) -> 'FasterGSVDAModel':
        """Builds the model."""
        self.gaussians = Gaussians(self.APPEARANCE)
        if self.PPISP.USE:
    ...
    def get_ply_dict(self) -> dict[str, np.ndarray | list[str]]:
        ...
        data['comments'] = [f'SplatRenderMode: {splat_render_mode}', 'Generated with NeRFICG/FasterGSVDA']
